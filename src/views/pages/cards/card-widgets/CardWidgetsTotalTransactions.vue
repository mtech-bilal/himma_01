...
<template>
  <VCard>
    <VRow no-gutters>
      <VCol cols="12" sm="7" :class="$vuetify.display.xs ? 'border-b' : 'border-e'">
        <VCardItem>
          <VCardTitle>Total Transactions</VCardTitle>
        </VCardItem>

        <VCardText>
          <VueApexCharts id="total-transactions-chart" type="bar" height="240" :options="chartConfig"
            :series="series" />
        </VCardText>
      </VCol>
      ...
      <VCol cols="12" sm="5">
        ...
        <div>
          <VCardText>
            <VRow no-gutters>
              <VCol v-for="(report, index) in reports" :key="report.title" cols="6" class="text-center"
                :class="index === 0 ? 'border-e' : ''">
                <VAvatar rounded="lg" variant="tonal" :color="Math.sign(report.stats) === 1 ? 'success' : 'primary'">
                  <VIcon
